import {
  ...
  logShimmedExport,
  logSourcePhaseFormatUnsupported,
  logSourcemapBroken,
  ...
  addEdit(sourceIndex, content, loc, nameIndex) {
    if (content.length) {
      ...
      while (contentLineEnd >= 0 && contentLengthMinusOne > contentLineEnd) {
        ...
        if (nameIndex >= 0) segment2.push(nameIndex);
  ...
  _replaceRegexp(searchValue, replacement) {
    ...
    if (searchValue.global) matchAll(searchValue, this.original).forEach((match) => {
      if (match.index != null) {
        ...
        if (replacement2 !== match[0]) this.overwrite(match.index, match.index + match[0].length, replacement2);
      }
    });
    else {
...
var SOURCE_PHASE_IMPORT = "*source";
...
var IdentifierBase = class extends NodeBase {
  ...
  isPossibleTDZ() {
    ...
    let declaration_id;
    if (this.variable.declarations && this.variable.declarations.length === 1 && (declaration_id = this.variable.declarations[0]) && this.start < declaration_id.start && closestParentFunctionOrProgram(this) === closestParentFunctionOrProgram(declaration_id)) {
...
function getReexportedImportName(moduleVariableName, imported, dependencyNamedExportsMode, isChunk, defaultVariableName, namespaceVariableName, interop, moduleId, externalLiveBindings, getPropertyAccess) {
  if (imported === "default") {
    ...
    return dependencyNamedExportsMode ? `${moduleVariableName}${getPropertyAccess("default")}` : moduleVariableName;
  }
  if (imported === "*") {
    return (isChunk ? !dependencyNamedExportsMode : namespaceInteropHelpersByInteropType[interop(moduleId)]) ? namespaceVariableName : moduleVariableName;
  }
...
function umd(magicString, { accessedGlobals, dependencies, exports, hasDefaultExport, hasExports, id, indent: t, intro, namedExportsMode, log, outro, snippets }, { amd: amd2, compact, esModule, extend, externalLiveBindings, freeze, interop, name, generatedCode: { symbols }, globals, noConflict, reexportProtoFromExternal, strict }) {
  ...
  warnOnBuiltins(log, dependencies);
  const amdDependencies = dependencies.map((m) => `'${updateExtensionForRelativeAmdId(m.importPath, amd2.forceJsExtensionForImports)}'`);
  ...
  const completeAmdId = getCompleteAmdId(amd2, id);
  const amdParameters = (completeAmdId ? `'${completeAmdId}',${_}` : ``) + (amdDependencies.length > 0 ? `[${amdDependencies.join(`,${_}`)}],${_}` : ``);
  ...
  if (noConflict) {
    ...
    if (!namedExportsMode && hasExports) {
      ...
      factory = `${cnst} ${noConflictExportsVariable}${_}=${_}${module};${n2}${t}${t}${factoryVariable}(${[noConflictExportsVariable, ...globalDependencies].join(`,${_}`)});`;
    }
  ...
  const cjsIntro = iifeNeedsGlobal ? `${t}typeof exports${_}===${_}'object'${_}&&${_}typeof module${_}!==${_}'undefined'${_}?${_}${cjsExport}${factoryVariable}(${cjsDependencies.join(`,${_}`)})${_}:${n2}` : "";
...
function requireParse() {
  ...
  const buildCharClassStar = (chars2) => {
...
function requirePicomatch$1() {
  ...
  picomatch2.matchBase = (input, glob, options, posix2 = options && options.windows) => {
    ...
    return regex.test(utils2.basename(input, { windows: posix2 }));
  };
...
var bufferParsers = [
  ...
  function importDeclaration(node, position, buffer) {
    ...
    node.phase = phaseIndex === 0 ? void 0 : FIXED_STRINGS[phaseIndex];
  },
...
function getVariableForExportNameRecursive(target, name, importerForSideEffects, isExportAllSearch, searchedNamesAndModules = /* @__PURE__ */ new Map(), importChain, sideEffectModules, exportOrReexportModules) {
...
var Module = class _Module {
  constructor(graph, id, options, isEntry, moduleSideEffects, syntheticNamedExports, meta, attributes) {
    ...
    this.sourcePhaseSources = /* @__PURE__ */ new Set();
  ...
  getVariableForExportName(name, { importerForSideEffects, importChain = [], isExportAllSearch, onlyExplicit, searchedNamesAndModules, sideEffectModules, exportOrReexportModules } = EMPTY_OBJECT) {
    ...
    if (reexportDeclaration) {
      ...
      if (this.info.moduleSideEffects) {
        sideEffectModules == null ? void 0 : sideEffectModules.add(this);
      }
      exportOrReexportModules == null ? void 0 : exportOrReexportModules.add(this);
      return [variable];
    }
    ...
    if (name !== "default") {
      ...
      if (foundNamespaceReexport[0]) {
        ...
        if (importerForSideEffects) {
          const { exportOrReexportModules: exportOrReexportModules2, sideEffectModules: sideEffectModules2 } = namespaceReexportOptions;
          for (const module of exportOrReexportModules2) {
          ...
          for (const module of sideEffectModules2) {
  ...
  addImport(node) {
    ...
    for (const specifier of node.specifiers) {
      ...
      const name = node.phase === "source" ? SOURCE_PHASE_IMPORT : specifier instanceof ImportDefaultSpecifier ? "default" : specifier instanceof ImportNamespaceSpecifier ? "*" : specifier.imported instanceof Identifier2 ? specifier.imported.name : specifier.imported.value;
  ...
  getVariableFromNamespaceReexports(name, importerForSideEffects, searchedNamesAndModules, importChain) {
    ...
    const sideEffectModules = /* @__PURE__ */ new Set();
    const exportOrReexportModules = /* @__PURE__ */ new Set();
    for (const module of this.exportAllModules) {
...
var concatSeparator = (out, next) => [out, next].filter(Boolean).join("\n");
var concatDblSeparator = (out, next) => [out, next].filter(Boolean).join("\n\n");
...
function makeUnique(name, { [lowercaseBundleKeys]: reservedLowercaseBundleKeys }) {
  ...
  const base2 = directory + (dotIndex === -1 ? fileName : fileName.slice(0, dotIndex));
  const extension = dotIndex === -1 ? "" : fileName.slice(dotIndex);
  let uniqueName, uniqueIndex = 1;
  while (reservedLowercaseBundleKeys.has((uniqueName = base2 + ++uniqueIndex + extension).toLowerCase()))
    ;
  return uniqueName;
}
...
var Chunk2 = class _Chunk {
  ...
  inlineTransitiveImports() {
    ...
    if (hoistTransitiveImports && !preserveModules && facadeModule !== null) {
      for (const dependency of dependencies) {
        if (dependency instanceof _Chunk)
...
`);
  ...
  getImportSpecifiers() {
    ...
    for (const variable of this.imports) {
      ...
      if (module instanceof ExternalModule) {
        ...
        if (!isSourcePhase && imported !== "default" && imported !== "*" && interop(module.id) === "defaultOnly") {
  ...
  getPreserveModulesChunkNameFromModule(module) {
    ...
    const sanitizedBase = sanitizeFileName2(this.inputBase);
  ...
  getRenderedDependencies() {
    ...
    for (const dependency of this.dependencies) {
      ...
      if (imports === null && reexports === null && dependency instanceof ExternalChunk && !dependency.moduleSideEffects && !this.outputOptions.hoistTransitiveImports) {
        continue;
      }
      ...
      const sourcePhaseImport = imports == null ? void 0 : imports.find((index) => index.phase === "source");
      const instanceImports = (imports == null ? void 0 : imports.filter((index) => index.phase !== "source")) ?? null;
      renderedDependencies.set(dependency, {
        ...
        reexports,
        sourcePhaseImport: sourcePhaseImport == null ? void 0 : sourcePhaseImport.local
      });
    }
...
function getChunkDefinitionsFromManualChunks(manualChunkAliasByEntry, isManualChunksFunctionForm, onlyExplicitManualChunks) {
  ...
  const manualChunkModulesByModule = /* @__PURE__ */ new Map();
  let index = 0;
...
function analyzeModuleGraph(entries, manualChunkModules, manualChunkModulesByModule) {
  ...
  for (const currentEntryModules of allEntriesAndManualChunks) {
    ...
    awaitedDynamicImportModulesByEntry[entryOrManualChunkIndex] = awaitedDynamicImportsForCurrentEntry;
    const staticDependencies = new Set(currentEntryModules);
    const addedManualChunks = /* @__PURE__ */ new Set();
...
var ModuleLoader = class {
  constructor(graph, modulesById, options, pluginDriver) {
    ...
    this.entryModules = /* @__PURE__ */ new Set();
...
var rollupVersion$2 = pkg.version;
...
var rollupVersion$1 = pkg.version;
...
async function watchInternal(configs, emitter) {
  ...
  await loadFsEvents();
  const { Watcher } = await import("./watch-S6HDFB7M.js");
...
rollup/dist/es/shared/node-entry.js:
  ...
  *)
*/
//# sourceMappingURL=chunk-4UDGU6EI.js.map
